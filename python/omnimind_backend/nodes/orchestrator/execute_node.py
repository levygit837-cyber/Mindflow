...
from omnimind_backend.nodes.base.streamable import StreamableNode
...
class ExecuteNode(StreamableNode, BaseNode):
    ...
    async def execute(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check for Decomposition Thinking Mode
        if decision.thinking_mode == ThinkingMode.DECOMPOSITION and settings.enable_decomposition_thinking:
            return await self._run_dt_pipeline(state, provider, model, memory_context)
        
        # Standard Execution Mode
        try:
            agent = get_agent(decision.agent)
            messages = self._build_messages(agent, state["message"], memory_context)
            
            # Set up sandbox and tools
            sandbox = self._setup_sandbox(agent, settings)
            registry = create_default_registry(sandbox)
            tools = self._get_tools_for_agent(agent, registry)
            
            # Get and configure LLM
            ...
            thought = self._extract_thoughts(response)
            if thought:
            ...
            self.set_node_state("tokens_used", self.get_node_state("tokens_used", 0) + self._count_tokens(response_text))
    ...
    async def _stream_execution(self, state: Dict[str, Any]) -> Any:
        ...
        from omnimind_backend.nodes.base.streamable import LLMStreamNode
    ...
    def _setup_sandbox(self, agent: Any, settings: Any) -> OmniMindSandbox:
        """Set up the sandbox for tool execution."""
        sandbox_root = getattr(settings, "working_path", None)
        return OmniMindSandbox(
