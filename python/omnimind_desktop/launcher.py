...
from omnimind_desktop.main import run_ui
...
def run() -> None:
    ...
    container_name = os.getenv("POSTGRES_CONTAINER_NAME", "omnimind-postgres-v1")
    db_user = os.getenv("POSTGRES_USER", "omnimind_app")
    db_name = os.getenv("POSTGRES_DB", "omnimind_v1")
    _wait_for_postgres(container_name, db_user, db_name)

    _run_step([sys.executable, "-m", "alembic", "upgrade", "head"], cwd=python_root)

    api_url = os.getenv("OMNIMIND_API_URL", "http://127.0.0.1:8000")
    ...
    if _truthy(os.getenv("OMNIMIND_START_GRPC")):
        grpc_process = _start_background_process(
            [sys.executable, "-m", "omnimind_backend.grpc.server"],
            cwd=python_root,
    ...
    try:
        ...
        if _truthy(os.getenv("OMNIMIND_DESKTOP_SKIP_UI")):
            return
