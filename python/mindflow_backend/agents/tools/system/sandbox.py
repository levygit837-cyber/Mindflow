"""Enhanced sandbox tool for secure command execution.

Migrated to the new ToolInterface architecture while preserving all
existing functionality including process isolation, timeouts, and security controls.
"""
...
class MindFlowSandbox(ToolInterface, BaseSandbox):
    ...
    Migrated to ToolInterface while preserving BaseSandbox inheritance
    and all existing security features including process isolation,
    timeouts, output limits, and read-only mode enforcement.
    """
    ...
    def __init__(
        ...
        """Initialize the sandbox with enhanced interface."""
        from pathlib import Path
        
        # Initialize ToolInterface
        super().__init__()
        self.name = "sandbox"
        self.description = "Secure sandbox for shell command execution with process isolation"
        
        # Initialize BaseSandbox
        BaseSandbox.__init__(self)
    ...
    async def execute(self, *args, **kwargs) -> Dict[str, Any]:
        """Execute command with enhanced interface and validation.
        ...
        try:
            ...
            working_dir = kwargs.get("working_directory", str(self.cwd))
            env = kwargs.get("environment", {})
            
            # Temporarily update working directory if specified
            ...
            if env:
                merged_env = {**self._env, **env}
                self._env = merged_env
            
            try:
                # Execute using existing BaseSandbox logic
    ...
    def _validate_command(self, command: str) -> tuple[bool, Optional[str]]:
        """Validate command for security and policy compliance.
        ...
        if self._read_only:
            ...
            if any(pat in cmd_lower for pat in write_patterns):
    ...
    def execute_command(
        ...
        *,
        ...
        """Execute a shell command within the sandbox (legacy interface).
        
        This method preserves the original BaseSandbox interface.
        """
        if not command:
    ...
    def download_files(self, paths: list[str]) -> list:
        """Download files from sandbox (legacy interface)."""
...
def get_sandbox_tool(**kwargs) -> MindFlowSandbox:
    """Factory function to get sandbox tool with configuration."""
