...
MIGRATION NOTE: This module is being migrated from LangChain tools to CallableTool.
The lc_tools parameter accepts CallableTool instances which are converted via callable_adapter.
"""
...
from mindflow_backend.runtime.execution.callable_adapter import callable_to_tool_definition
...
class ToolOrchestrator:
    ...
    def __init__(
        self,
        *,
        lc_tools: list[Any],  # Accepts CallableTool instances (migration in progress)
        ...
        # Convert CallableTools to internal format if needed
        self._lc_tools = lc_tools
        self._tools_by_name: dict[str, Any] = {}

        # Try to detect if these are CallableTools and convert them
        for tool in lc_tools:
            tool_name = getattr(tool, 'name', None)
            if tool_name:
                # Check if it's a CallableTool (has call method)
                if hasattr(tool, 'call'):
                    # Convert CallableTool to internal format
                    from mindflow_backend.schemas.tools.callable import CallableTool
                    if isinstance(tool, CallableTool):
                        # Store the original CallableTool
                        self._tools_by_name[tool_name] = tool
                        continue
                # Fallback: store as-is (legacy LangChain support)
                self._tools_by_name[tool_name] = tool
    ...
    async def _execute_tool_call(
        ...
        try:
            ...
            # Check if this is a CallableTool
            from mindflow_backend.schemas.tools.callable import CallableTool
            if isinstance(tool, CallableTool):
                # Use CallableTool.call() method
                from mindflow_backend.schemas.tools.context import ToolContext

                # Create ToolContext (minimal for now)
                ...
                try:
                    ...
                    serialized = json.dumps(
                        {"success": False, "error": f"Input validation failed: {e}"},
                        ensure_ascii=False,
                    )
                    return OrchestratedToolCallResult(
                        tool_call_id=tool_call_id,
                ...
                # Invoke the tool
                result = await tool.call(validated_input, tool_context, on_progress=None)

                # Convert ToolResult to serialized format
                if result.success:
                    serialized_result = self._serialize_tool_result(result.data)
                else:
                ...
                return OrchestratedToolCallResult(
                    ...
                    tool_name=tool_name,
                    tool_args=tool_args,
                    raw_result=result.data if result.success else None,
                    serialized_result=serialized_result,
                    tool_meta=tool_meta,
                    error=result.error if not result.success else None,
                )
            else:
                # Fallback: use LangChain-style ainvoke
