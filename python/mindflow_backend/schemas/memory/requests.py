...
class MemoryStoreRequest(BaseModel):
    ...
    token_start: Optional[int] = Field(default=None, ge=0, description="Start token position")
    token_end: Optional[int] = Field(default=None, ge=0, description="End token position")
...
class MemoryRetrieveRequest(BaseModel):
    ...
    session_id: UUID = Field(description="Session identifier")
    memory_ids: Optional[List[UUID]] = Field(default=None, description="Specific memory IDs to retrieve")
...
class MemoryUpdateRequest(BaseModel):
    ...
    # Position updates
    token_start: Optional[int] = Field(default=None, ge=0, description="Updated start position")
    token_end: Optional[int] = Field(default=None, ge=0, description="Updated end position")
    
    # Relationship updates
    parent_id: Optional[UUID] = Field(default=None, description="Updated parent")
    related_ids: Optional[List[UUID]] = Field(default=None, description="Updated relationships")
    
    # Expiration
    expires_at: Optional[datetime] = Field(default=None, description="Updated expiration")
    
    # Embedding options
    regenerate_embedding: bool = Field(default=False, description="Whether to regenerate embedding")
    embedding_model: Optional[str] = Field(default=None, description="New embedding model")
...
class ContextWindowRequest(BaseModel):
    ...
    position: Optional[int] = Field(default=None, ge=0, description="Center position for window")
    ...
    memory_types: Optional[List[MemoryType]] = Field(default=None, description="Memory types to include")
...
class MemoryCursorRequest(BaseModel):
    ...
    position: Optional[int] = Field(default=None, ge=0, description="Cursor position")
    anchor_token: Optional[int] = Field(default=None, ge=0, description="Anchor token")
    
    # Context options
    context_window: Optional[int] = Field(default=None, ge=1, le=10000, description="Context window size")
    retrieval_strategy: RetrievalStrategy = Field(default=RetrievalStrategy.TEMPORAL)
    
    # Movement parameters (for move operation)
    direction: Optional[str] = Field(default=None, pattern="^(forward|backward)$", description="Move direction")
    distance: Optional[int] = Field(default=None, ge=1, description="Move distance")
...
class MemoryStatsRequest(BaseModel):
    ...
    # Temporal range
    start_date: Optional[datetime] = Field(default=None, description="Statistics start date")
    end_date: Optional[datetime] = Field(default=None, description="Statistics end date")
...
class MemoryImportRequest(BaseModel):
    ...
    field_mappings: Optional[Dict[str, str]] = Field(default=None, description="Field name mappings")
