...
async def run_workers(worker_types: list[str] | None = None) -> None:
    ...
    try:
        ...
        if hasattr(monitor, "collect_once"):
            await monitor.collect_once()
        if hasattr(monitor, "get_health_report"):
            health_report = monitor.get_health_report()
            if hasattr(health_report, "model_dump"):
                _logger.info(
                    "Worker startup health snapshot",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="MindFlow Worker Process")
    parser.add_argument(
        "--workers",
        nargs="+",
