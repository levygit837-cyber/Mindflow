...
class TestProposalEvaluator:
    ...
    @pytest.mark.asyncio
    async def test_no_proposals_returns_direct_response(
        self, evaluator: "ProposalEvaluator"
    ) -> None:
        """When no proposals, fallback to DIRECT_RESPONSE."""
    ...
    async def test_filter_all_unhealthy_fallback(
        ...
        """If all agents are unhealthy, fallback to DIRECT_RESPONSE."""
        proposals = [
            AgentProposal(
                agent_id="coder",
                confidence=0.9,
