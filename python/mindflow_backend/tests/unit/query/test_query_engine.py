...
class TestWorkflowStepConversion:
    ...
    def test_workflow_step_to_delegation_task_basic(self):
        ...
        step = WorkflowStep(
            ...
            specialist=SpecialistType.FRONTEND,
        ...
        assert task.specialist == SpecialistType.FRONTEND
        assert task.agent_id == "coder:frontend"
        assert task.objective == "Implement the login form"
        assert task.session_id == "test_session"
        assert task.tools == ["read_file", "write_file"]
    ...
    def test_delegation_result_to_step_output(self):
        ...
        step = WorkflowStep(
            step_id="step_1",
            agent_role=AgentType.CODER,
            specialist=SpecialistType.BACKEND,
        ...
        assert output["agent_id"] == "coder:backend"
        assert output["agent_role"] == "coder"
        assert output["specialist"] == "backend"
...
class TestQueryEngineDelegationTask:
    ...
    async def test_delegate_task_basic(self):
        """Test basic delegation task execution."""
        engine = QueryEngine(
            providers=[],
            budget=TokenBudget(max_tokens=100_000),
            session_id="test_session",
        ...
        # Mock the _delegation_primary method
        mock_result = DelegationResult(
            task_id="task_1",
            agent=AgentType.CODER,
            agent_role=AgentType.CODER,
            ...
            agent_id="coder",
            status="completed",
            key_findings="Function written",
            full_output="def example():\n    pass",
            confidence=0.9,
            tokens_consumed=500,
        )

        with patch.object(engine, "_delegation_primary", new_callable=AsyncMock, return_value=mock_result):
            with patch.object(engine, "_start_child_execution", new_callable=AsyncMock, return_value=MagicMock(id="exec_1")):
                with patch.object(engine, "_resolve_task_workspace", new_callable=AsyncMock, return_value=task):
        ...
        assert result.status == "completed"
        assert result.key_findings == "Function written"
